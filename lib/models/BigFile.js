...
module.exports = function(sequelize, BigFileLink, config) {
	...
	// Locals =========================================================
	const LINK_TYPE = config.get('LINK_TYPE');
	const fileStorage = new FileStorage(config.get('FILE_PATH'));
	const tempFileStorage = new FileStorage(config.get('TEMP_FILE_PATH'));
	...
	let classMethods = {
		...
		hasQuota: function*(req, type) {
			...
			return usedQuota <= config.get('USER_QUOTA').toKB();
		...
		upload: function*(req, res, linkType) {
			...
			yield skipperFile.uploadAsync({saveAs: function (__newFileStream, cb) {
				...
				maxBytes: config.get('MAX_FILE_SIZE').toKB()
			});
		...
		link: function*(uuid, type, referredBy) {
			...
			return co(function* () {
				const file = yield self.findByUUID(uuid);
